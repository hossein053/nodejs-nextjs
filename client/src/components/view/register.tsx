...
export const Register = ({ setShow }: { setShow: (value: boolean) => void }) => {
        ...
        try {
            await fetch(`${URL}/register`, {
                ...
                if (response.status === 201) {
                    toast.success('Registration was successful', { theme: 'dark' })
                }
            });
        } catch (error) {
    ...
    return (
        <div className="z-[1] h-full relative p-7 pt-12">
            <form className="w-10/12 grid grid-cols-4 gap-2" onSubmit={handleSubmit}>
                ...
                <button onClick={handler} type='button' className="row shadow-sx border border-purple_25 px-10 py-2 bg-white text-purple_75 font-semibold text-lg col-span-3 whitespace-nowrap rounded-md mt-2">
